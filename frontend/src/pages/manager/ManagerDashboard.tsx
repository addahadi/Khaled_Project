    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <Stat label="Doctors"        value={reports?.staffCounts.doctors}   icon={Building2}   color="text-blue-500"    sub="Active doctors" />
          <Stat label="Lab Technicians" value={reports?.staffCounts.lab_techs} icon={FlaskConical} color="text-teal-500" sub="Lab staff" />
      ...
      <div>
        <h2 className="text-sm font-semibold text-muted-foreground uppercase tracking-wide mb-3">Activity (Last 30 Days)</h2>
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          <Stat label="Total Lab Tests"   value={reports?.labStats.total_tests}       icon={FlaskConical}    color="text-primary"    />
          <Stat label="Pending Tests"     value={reports?.labStats.pending}           icon={AlertTriangle}   color="text-yellow-500" />
          <Stat label="Critical Results"  value={reports?.labStats.critical_results}  icon={AlertTriangle}   color="text-destructive" />
          <Stat label="AI Predictions"    value={reports?.predStats.total_predictions} icon={Brain}          color="text-indigo-500" />
        </div>
      </div>
