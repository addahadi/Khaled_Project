...
export default function Subscription() {
 ...
 <h1 className="text-[28px] font-light">Subscription</h1>
 ...
 <span className="text-[32px] font-normal">{subscription.plan_name}</span>
 ...
 {/* Available plans */}
 <div>
 <h2 className="text-lg font-normal mb-4">Available Plans</h2>
 {plansLoading ? (
 ...
 <div className={`flex h-9 w-9 items-center justify-center  ${isCurrent ? 'bg-primary' : 'bg-muted'}`}>
 ...
 {plan.price_monthly ? (
 <>
 <span className="text-[32px] font-normal">${plan.price_monthly}</span>
 <span className="text-muted-foreground text-sm mb-1">/mo</span>
 </>
 ) : (
 <span className="text-xl font-normal text-muted-foreground">Free Trial</span>
 )}
 </div>
 ...
 <CheckCircle2 className={`h-3.5 w-3.5 shrink-0 ${f.is_enabled ? 'text-[#24a148]' : 'text-muted-foreground'}`} />
 {FEATURE_LABELS[f.name] ?? f.name}
 {f.value && ` (${f.value})`}
 </li>
 ))}
 ...
 <div className="space-y-4">
 <h2 className="text-lg font-normal">Billing History</h2>
 <Card>
 ...
 {/* Current */}
 <div className=" border p-3 bg-muted/30 space-y-2">
 <p className="text-xs font-medium text-muted-foreground uppercase tracking-wide">Current</p>
 <p className="font-normal text-sm">{subscription.plan_name}</p>
 ...
 {/* New */}
 <div className=" border-2 border-primary p-3 bg-primary/5 space-y-2">
 <p className="text-xs font-medium text-primary uppercase tracking-wide">New</p>
 <p className="font-normal text-sm">{confirmPlan.name}</p>
 <p className="text-xl font-normal">
 ...
 diff > 0
 ? 'bg-orange-50 text-[#ff832b] dark:bg-orange-950/20'
 ...
 {gained.map(f => (
 <div key={f.name} className="flex items-center gap-1.5 text-[#24a148]">
