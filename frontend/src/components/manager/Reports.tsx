...
function StatCard({ title, value, icon: Icon, color }: {
 ...
 <CardContent>
 <div className="text-[32px] font-normal">{value ?? '—'}</div>
 </CardContent>
 </Card>
 );
}
...
export default function Reports() {
 ...
 <h1 className="text-[28px] font-light">Reports & Analytics</h1>
 ...
 <StatCard title="Doctors"  value={reports?.staffCounts.doctors}  icon={Users}  color="text-primary" />
 <StatCard title="Lab Technicians"  value={reports?.staffCounts.lab_techs} icon={FlaskConical} color="text-primary" />
 ...
 {/* Lab activity */}
 <section>
 <h2 className="text-sm font-normal text-muted-foreground uppercase tracking-wide mb-3">Lab Activity</h2>
 <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
 <StatCard title="Total Tests"  value={reports?.labStats.total_tests}  icon={FlaskConical}  color="text-primary" />
 <StatCard title="Pending Tests"  value={reports?.labStats.pending}  icon={AlertTriangle} color="text-[#a2680a]" />
 <StatCard title="Completed Tests"  value={reports?.labStats.completed}  icon={FlaskConical}  color="text-[#24a148]" />
 ...
 {/* Predictions */}
 <section>
 <h2 className="text-sm font-normal text-muted-foreground uppercase tracking-wide mb-3">AI Predictions</h2>
 <div className="grid gap-6 lg:grid-cols-2">
 <div className="grid gap-4 sm:grid-cols-2">
 <StatCard title="Total Predictions" value={reports?.predStats.total_predictions} icon={Brain} color="text-primary" />
 <StatCard title="Critical"  value={reports?.predStats.critical}  icon={AlertTriangle} color="text-destructive" />
 <StatCard title="High Risk"  value={reports?.predStats.high}  icon={AlertTriangle} color="text-[#ff832b]" />
 <StatCard title="Low Risk"  value={reports?.predStats.low}  icon={Brain}  color="text-[#24a148]" />
 </div>
 ...
 {reports?.usageStats && (
 <section>
 <h2 className="text-sm font-normal text-muted-foreground uppercase tracking-wide mb-3">Subscription Usage (Current Cycle)</h2>
 <div className="grid gap-4 sm:grid-cols-3">
 <Card>
 <CardHeader className="pb-2"><CardTitle className="text-sm text-muted-foreground">Predictions Used</CardTitle></CardHeader>
 <CardContent>
 <div className="text-[32px] font-normal">{reports.usageStats.prediction_used}</div>
 ...
 <CardContent>
 <div className={`text-[32px] font-normal ${reports.usageStats.prediction_overage > 0 ? 'text-destructive' : ''}`}>
 ...
 <CardContent>
 <div className="text-[32px] font-normal text-[#24a148]">
 {reports.usageStats.prediction_limit
